...
test('注册提示用户名和六位密码规则且恢复码只显示到确认保存', async ({ page }) => {
  ...
  await expect(page.getByText('用户名支持中文并区分大小写；用户名无应用级长度限制，密码至少6位。'))
    .toBeVisible();

  const usernameInput = page.getByLabel('用户名');
  const passwordInput = page.getByLabel('密码', { exact: true });
  const passwordConfirmInput = page.getByLabel('确认密码');
  await expect(usernameInput).toHaveAttribute('placeholder', '请输入你的用户名');
  await expect(passwordInput).toHaveAttribute('placeholder', '请输入至少6位密码');
  await expect(passwordInput).toHaveAttribute('minlength', '6');
  await expect(passwordConfirmInput).toHaveAttribute('placeholder', '请输入至少6位密码');
  await expect(passwordConfirmInput).toHaveAttribute('minlength', '6');

  await usernameInput.fill(username);
  await passwordInput.fill(password);
  await passwordConfirmInput.fill('Different-Horse-2026');
  await page.getByRole('button', { name: '创建账号' }).click();
...
test('注册登录后完成四步会先显示报告再自动保存并可查看只读历史详情', async ({ page }) => {
  ...
  await expect(page.locator('.history-detail')).toContainText('截止：今天18:00');
  await expect(page.locator('.history-detail')).toContainText('约1h');
  await expect(page.locator('.history-detail')).not.toContainText('截止：年底');
