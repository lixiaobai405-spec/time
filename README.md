...
- AI 拆解后由用户编辑任务名称、类别、预估工时和轻重缓急；截止日期不在前端展示或编辑，后端仍兼容模型识别和历史数据中的 `due`。服务端执行逐字段 SMART 门禁，任务具体、工时可解析且轻重缓急完整即可继续。
...
`due` 作为可选兼容字段继续保存在任务和历史 JSON 中；缺失、空白或 `null` 会标准化为"待确认"，本次无需数据库迁移。
