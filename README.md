...
面向管理者的自我管理智能体。它用“昨天—今天—明天—后天”梳理复盘与目标，再提取任务、落入重要/紧急矩阵并生成行动报告。

## 当前状态

- `frontend/index.html` 是静态交互原型；Markdown 解析器已固定版本并随项目离线打包，预览时不请求 CDN。
- 原型目前没有接入大模型或后端；任务提取、矩阵和报告均为固定演示内容，用户输入与手动任务尚未真正传递到后续步骤。
- `prompts/system.md` 是当前 v2 运行真源，其中已内嵌 PDCA、SMART 和重要/紧急矩阵规则。
...
├─ prompts/system.md         # v2 全流程提示词（含方法库）
├─ tests/prompt-cases.md     # v2 提示词测试用例
├─ knowledge/README.md       # 知识库边界说明
└─ docs/legacy-combined.md   # 旧版合并稿，仅供追溯
```

`docs/legacy-combined.md` 缺少 v2 新增的 PDCA/SMART 检查规则，不得用于生产配置。
...
生产接入仍须先校验提示词约定的 JSON Schema，再逐字段渲染；象限、优先级、耗时等原子字段继续使用 `textContent`，不能把模型整段响应直接当 Markdown。

## 后端接入边界

真实实现需要按 `prompts/system.md` 串联四次结构化调用：目标检查、任务提取、矩阵判定、报告生成。每步只接收约定字段并返回约定 JSON；解析失败最多重试一次。用户输入可能包含客户、绩效与日程信息，默认只在当前会话内使用，不写入日志或跨会话记忆。
