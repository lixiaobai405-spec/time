...
- [ ] 激活项目专用 `.conda`，确认未使用全局 Python，且测试没有真实模型网络调用。
- [ ] 运行完整测试：
...
- [ ] 检查 Git hooks（钩子）配置并执行项目实际存在的 hooks；不得修改、禁用或使用 `--no-verify` 绕过：
...
- [ ] 审计敏感信息与提交范围，只检查变量名和文件名，不读取 `.env` 值：
...
- [ ] 确认启动脚本相关既有改动仍保持原状，未被纳入本计划提交。
- [ ] 勾选 Task 12；若勾选本身形成最后一个文档改动，单独提交：
...
- [ ] 所有 Task 1–12 均已按 TDD 完成并勾选。
- [ ] 明确当天或已逾期的 AI 提取任务统一为 `urgency="高"`；“待确认”和不可解析日期不被猜测。
- [ ] 矩阵仍只把“高”映射为重要或紧急，且不改写已有人工/提取标签。
- [ ] 报告顺序可由服务端确定性复现，16:00 必须早于 17:00。
- [ ] 当天或逾期任务不会被建议推迟、延后、取消、暂缓或搁置。
- [ ] 当天第三象限任务获得授权/委派建议；超过五条的当天任务不会从报告中静默消失。
- [ ] 中短期 SMART 任务保留可见验收标准；超过 8h 的中长期任务显示明确下一步。
- [ ] 稳定任务 ID 继续用于结构化关联，但报告页面、模型可见文字字段和复制内容均不包含完整 ID 或当前 UUID 的 8 位前缀。
- [ ] 稳定 ID、截止时间“待确认”、55/25/15/5、已完成事实不生成待办等原正式契约全部回归通过。
- [ ] `npm test`、`git diff --check`、实际存在的 Git hooks 全部通过，有新鲜命令输出作为证据。
- [ ] `git status` 中没有意外文件、敏感信息、缓存、日志或测试产物；已有启动脚本改动未被覆盖或混入。
- [ ] 最终汇报包含完成任务、修改文件、逐项测试结果、未完成事项、风险、Git 状态和英文提交记录。
