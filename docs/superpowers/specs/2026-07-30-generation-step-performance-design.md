...
```text
prompts/decomposition/evidence-task-generation.v2.md
```
...
6. 每个 `planned` 或 `unfinished` evidence 必须成为至少一项任务的主证据；
...
11. 最后执行 UUID、日期归一化、紧迫度和 SMART 检查。
...
- 现有 16 个 replay 案例全部通过；
- 每个非空输入行均被 evidence 覆盖；
- 所有 planned/unfinished evidence 均有主任务；
